...
public class AzurePanel implements AzureAbstractConfigurablePanel {
    ...
    private void saveAzConfig() {
        final AzureConfiguration config = Azure.az().config();
        IApplicationStore appStore = AzureStoreManager.getInstance().getAppStore();
        appStore.setProperty(AzureConfigInitializer.TELEMETRY, AzureConfigInitializer.TELEMETRY_ALLOW_TELEMETRY,
                Boolean.toString(config.getTelemetryEnabled()));
        appStore.setProperty(AzureConfigInitializer.ACCOUNT, AzureConfigInitializer.AZURE_ENVIRONMENT_KEY,
                config.getCloud());
        appStore.setProperty(AzureConfigInitializer.FUNCTION, AzureConfigInitializer.FUNCTION_CORE_TOOLS_PATH,
                config.getFunctionCoreToolsPath());
        appStore.setProperty(AzureConfigInitializer.DATABASE, AzureConfigInitializer.PASSWORD_SAVE_TYPE,
