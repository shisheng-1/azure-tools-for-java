...
public class AzureConfigInitializer {
    ...
    public static void initialize() {
        ...
        String installId = AzureStoreManager.getInstance().getFileStore().getProperty(TELEMETRY,
                TELEMETRY_INSTALLATION_ID);
        if (StringUtils.isBlank(installId) || InstallationIdUtils.isValidHashMac(installId)) {
            // old settings 1
            installId = AzureStoreManager.getInstance().getAppStore().getProperty(null, TELEMETRY_INSTALLATION_ID);
        }
        ...
        AzureStoreManager.getInstance().getFileStore().setProperty(TELEMETRY,
                TELEMETRY_INSTALLATION_ID, installId);

        final IApplicationStore appStore = AzureStoreManager.getInstance().getAppStore();
        ...
        appStore.setProperty(TELEMETRY, TELEMETRY_ALLOW_TELEMETRY, config.getTelemetryEnabled().toString());

        final String azureCloud = migrateAndGetSetting(ACCOUNT, AZURE_ENVIRONMENT_KEY, "Azure");
        config.setCloud(azureCloud);
        appStore.setProperty(ACCOUNT, AZURE_ENVIRONMENT_KEY, azureCloud);
        ...
        if (isDataFileValid()) {
            ...
            try {
                ...
                if (StringUtils.isNoneBlank(installationId, pluginVersion)) {
                    AzureStoreManager.getInstance().getAppStore().setProperty(TELEMETRY, TELEMETRY_ALLOW_TELEMETRY, Boolean.toString(allowTelemetry));
                    AzureStoreManager.getInstance().getAppStore().setProperty(TELEMETRY, TELEMETRY_PLUGIN_VERSION, pluginVersion);
