...
public class AuthMethodManager {
    ...
    public void persistAuthMethodDetails() {
        ...
        try {
            ...
            AzureStoreManager.getInstance().getAppStore().setProperty(ACCOUNT, AUTH_METHOD_DETAIL, sd);
    ...
    private static AuthMethodDetails loadSettings() {
        ...
        try {
            String json = AzureStoreManager.getInstance().getAppStore().getProperty(ACCOUNT, AUTH_METHOD_DETAIL, "");
            if (StringUtils.isBlank(json)) {
                FileStorage fs = new FileStorage(FILE_NAME_AUTH_METHOD_DETAILS, CommonSettings.getSettingsBaseDir());
                byte[] data = fs.read();
                json = new String(data);
                AzureStoreManager.getInstance().getAppStore().setProperty(ACCOUNT, AUTH_METHOD_DETAIL, json);
                fs.removeFile();
            }
            if (StringUtils.isBlank(json)) {
